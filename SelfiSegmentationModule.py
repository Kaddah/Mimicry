...
class SelfiSegmentation():
    ...
    def get_segments(self, img, imgBg, cutThreshold=0.1):
        ...
        condition = results.segmentation_mask > cutThreshold
        condition = np.stack((condition,) * 3, axis=-1)
        
        emptyFrame = np.zeros_like(img, dtype = np.uint8) # typ rgb bild? Opencv cvtCOLOR RGB oder so 
        #es müsste int array sein 
